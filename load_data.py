...
def load_signals_CHBMIT(data_dir, target, data_type):
    ...
    for filename in filenames:  # 特定患者特定时期
        ...
        raweeg = read_raw_edf('%s/%s' % (chb_dir, filename), verbose=0, preload=True)  # 预加载（preload=Ture）
        # raweeg.plot()
        ...
        if data_type == 'ictal':  # 发作期及发作前期处理
            ...
            if len(indices) > 0:  # 判断有没有发作期数据
                ...
                for i in range(len(indices)):
                    st = szstart[indices[i]] * 256 - 1 * 60 * 256  # SPH=1min  发作开始前五分钟（szstart和szstop记录的单位是秒数）可能小于零（从开始记录到发作不到五分钟）
...
class PrepData:  # class PreData():
    ...
    def preprocess_Kaggle(self, data_):
        ...
        def process_raw_data(mat_data):
            ...
            for segment in mat_data:
                ...
                if data.shape[1] < 16:  # if the channels smaller than 16, add to 16 channels (in this code, 15 channels
    ...
    def apply(self):
        ...
        os.makedirs(self.settings['cachedir'], exist_ok=True)
        save_hickle_file(
            os.path.join(self.settings['cachedir'], filename),
            [x, y])
        return x, y
...
if __name__ == '__main__':
    ...
    # TODO change samples len, go to lines 444
